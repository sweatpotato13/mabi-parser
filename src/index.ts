        ...
        try {
            ...
            if (finalSave) {
                ...
                if (uniqueItems.length < minAcceptableItems) {
                    console.log(
                        `수집된 아이템 수(${uniqueItems.length})가 최소 기준(${minAcceptableItems})보다 적습니다.`,
                    );
                    ...
                    console.log(
                        `총 ${uniqueItems.length}개의 아이템이 성공적으로 저장되었습니다.`,
                    );
                    // 백업 파일 삭제
                    if (fs.existsSync(backupFilename)) {
    ...
    try {
        ...
        while (true) {
            ...
            const items: Item[] = await page.evaluate(() => {
                ...
                return Array.from(
                    container.querySelectorAll(".v-card.v-theme--dark"),
                )
                    .map(card => {
                        const fullText = (
            ...
            await page.evaluate(() => {
                ...
                const virtualItems = container.querySelectorAll(
